...
import { addLocation } from "@/app/actions/savedLocations";
import { setFavourite } from "@/app/actions/setFavourite"; 
...
export default function FavouriteLocationCard() {
  ...
  // --- title / tz helpers ---
  const title = useMemo(
    () => fav?.display_name || (userId ? "Choose your favourite location" : "Favourite Location"),
    [fav?.display_name, userId]
  );
  const { tz, tzOffset } = bundle ? getTz(bundle) : { tz: null as string | null, tzOffset: null as number | null };
  const formatHM = (unix?: number) => {
    if (!unix) return "—";
    const base = new Date(unix * 1000);
    if (tz) {
      try { return base.toLocaleTimeString("en-GB", { timeZone: tz, hour: "2-digit", minute: "2-digit" }); } catch {}
    }
  ...
  const weekdayFor = (unix: number) => {
    const base = new Date(unix * 1000);
    if (tz) {
      try { return base.toLocaleDateString("en-GB", { weekday: "short", timeZone: tz }); } catch {}
    }
    if (typeof tzOffset === "number") {
      const shifted = new Date((unix + tzOffset) * 1000);
  ...
  // --- Add new favourite using your existing flow ---
  // Reuse AddLocationModal → it returns { id, displayName, lat, lon }
  const handleAddAsFavourite = async (r: { id: string; displayName: string; lat: number; lon: number }) => {
    if (!userId) { setError("Please sign in to set a favourite location."); return; }
    setError(null);
    setLoading(true);
    try {
      // 1) ensure it's saved (same as SavedLocations does)
      await addLocation({
        location_id: r.id,
      ...
      // 2) mark it as favourite (and unset previous)
      await setFavourite(r.id);
      // 3) reflect in UI
      setFav({
        location_id: r.id,
        display_name: r.displayName,
  ...
  return (
    <>
      <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between relative z-10">
        ...
        {/* Reuse your existing search/add modal */}
        <AddLocationModal
      ...
      <Card style={surfaceVars} className="mt-6 rounded-2xl backdrop-blur-md border shadow-md">
        ...
        <div className="p-4">
          ...
          {!loading && !error && !fav && userId && (
            <p className="opacity-80">Pick a location using the “Add location” button.</p>
          )}
        </div>
      </Card>
    </>
